...
genre_dictionnary = requests.get(
    "https://api.themoviedb.org/3/genre/movie/list?language=en", headers=headers
).json()["genres"]
...
def get_movies_info(ids=[], headers=None):
    ...
    for i in tqdm(ids):
        ...
        dataframes.append(
            pd.json_normalize(req)
        )  # pas très efficace voir si on peut améliorer
...
def keep_main_genre(df=None):
    ...
    if isinstance(
        df["genres"].iloc[0], str
    ):  # in case we open a csv flight where the dictionnaries were transformed in str.
        df["genres"] = df["genres"].apply(ast.literal_eval)
    df = df.dropna(subset=["genres"])
    df = df[
        df["genres"].apply(lambda x: len(x) > 0)
    ]  # get rid of ligns ith no genre (encoded: "")
